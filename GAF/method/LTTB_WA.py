...
def LTTB_WA(data, threshold):
    """
    改进后的LTTB方法，需要调节多个参数来改进策略
    返回一个数据规约后的时序数据
    Parameters
    ----------
    ...
    for i in range(0, threshold - 2):
        ...
        while range_offs < range_to:
            ...
            if area > max_area:
                max_area = area
                submax_area_point = data[range_offs]
                max_area_point[0] = submax_area_point[0]
                max_area_point[1] = 0.2 * submax_area_point[1] + 0.8 * max_area_point[1]#+0.1*sum_point_value
                print(max_area_point)
                # max_area_point = 0.8*submax_area_point+0.2*max_area_point
                next_a = range_offs  # Next a 就是 range_offs
            max_area_point[1] = 0.8*max_area_point[1]+0.2*sum_point_value
            range_offs += 1
